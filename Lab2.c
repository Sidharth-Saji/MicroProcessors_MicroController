...
/*
	Two Seven Segment LED's: Display numbers from 00 to 99
*/
	...
	while (1)
	{
		...
		for (int p = 0; p < 99; p++)
		{
			...
			for (int n = 0; n < delay / 1000; n++)
			{
				for (int l = 0; l < 500; l++)
				{
					GPIOE_PDOR = 0x2; // Selects any one of the two Sev-Seg Display
					GPIOC_PDOR = val[k];
				}
				for (int l = 0; l < 500; l++)
				{
					GPIOE_PDOR = 0x1; // Selects any one of the two Sev-Seg Display
					GPIOC_PDOR = val[j];
				}
			}
		}
